...
const CartPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const newTotal = newSubtotal + newTax + (cartItems.length > 0 ? 4.99 : 0);

    setSubtotal(newSubtotal);
    setTax(newTax);
    setShipping(cartItems.length > 0 ? 4.99 : 0);
    setTotal(newTotal);
  ...
  const handleApplyCoupon = () => {
    alert(`Promo code "${couponCode}" applied (demo only).`);
  };
  ...
  return (
    <div className="text-gray-800 font-sans bg-[linear-gradient(135deg,#22c55e_0%,#f97316_100%)] bg-fixed min-h-screen flex flex-col">
            ...
            <section className="w-full lg:w-2/3 space-y-6">
              {cartItems.length > 0 ? (
                cartItems.map((item, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between bg-green-50 border border-green-200 p-4 rounded-lg shadow-sm"
                  >
                    <div className="flex items-center gap-4">
      ...
      <footer className="bg-green-800 text-gray-100 py-6 mt-10 text-center text-sm">
        <p>
          &copy; 2025 <span className="text-orange-400 font-semibold">Allen City Pharmacy</span>. All
          Rights Reserved.
        </p>
      </footer>
    </div>
